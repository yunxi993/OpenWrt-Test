...
# Hostname
sed -i 's/OpenWrt/nzksma/g' package/base-files/files/bin/config_generate
...
echo 'iptables -A INPUT -s 17.57.144.0/22 -p tcp -m multiport --sports 5223,443 -j DROP' >> /etc/firewall.user\n\
echo 'iptables -A OUTPUT -d 17.57.144.0/22 -p tcp -m multiport --dports 5223,443 -j DROP' >> /etc/firewall.user
" package/lean/default-settings/files/zzz-default-settings
